...
- **Stage 9 (drop `Chord` from Element variant) ALSO deferred.** Stage 7 lands chord-event emit through `BlockRealizationStrategy` (which still emits Chord-typed Elements). Dropping Chord requires either reproducing `ChordPerformer::perform_block`'s 10ms-per-pitch spread + humanize at compose time (crosses the "humanize is perform-tier" line) or re-engineering the perform tier to apply spread to consecutive simultaneous Notes. Either path goes beyond the refactor's scope; chord-walker integration was the natural justification for that work and chord-walker is deferred. Chord stays in the Element variant; ChordPerformer keeps owning chord-attack semantics. Compose-tier output remains authoritative — the Chord Element IS score-data, just not fully expanded into per-tone Notes.
- **Stage 11 (delete `ChordAccompanimentConfig`)** is partially reduced: the rhythm-pattern half dissolves into RhythmPattern at Stage 10, but the voicing-hint half (octave/inversion/spread) stays. Decide at Stage 11 execution time whether to slim the struct down or lift its remaining fields directly onto PassageTemplate.
